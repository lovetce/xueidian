...
<Nonce><![CDATA[%s]]></Nonce>
	...
	/**
	 * 获取access_token 首先从缓存里找，找不到再到数据库里找，再找不到就从微信服务器获取
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		//从数据库里获取
		//todo
		$data = $this->database->get("token",array("expire_time","token(access_token)"),array("AND" => array("appid"=>$appid,'type'=>'token') ));
		if($data){
			$curr_time = time();
			if($data['expire_time']>$curr_time){
				$this->access_token = $data['access_token'];
				//设置缓存 缓存时间 = 有效时间-当前时间
				$this->setCache($authname,$this->access_token,($data['expire_time']-time()) );
		...
		if ($result)
		{
			...
			//更新数据库
			if($data){
				$this->database->update("token", array("expire_time"=>(time()+$expire),"token"=>$this->access_token),array("AND" => array("appid"=>$appid,'type'=>'token')));
			}else{
				$this->database->query("INSERT INTO iweite_huace_token (expire_time,token,appid,type) VALUES ('".(time()+$expire)."','$this->access_token','$appid','token')");
			}
			return $this->access_token;
		}
		return false;
	...
	/**
	 * 获取JSAPI授权TICKET(有效时间7200秒) 首先从缓存里找，找不到再到数据库里找，再找不到就从微信服务器获取
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
		...
		//2从数据库中找
		//todo
		$data = $this->database->get("token",array("expire_time","token(jsapi_ticket)"),array("AND" => array("appid"=>$appid,'type'=>'ticket') ));
		if($data){
			if($data['expire_time']>time()){
				$this->jsapi_ticket = $data['jsapi_ticket'];
				//设置缓存 缓存时间 = 有效时间-当前时间
				$this->setCache($authname,$this->jsapi_ticket,($data['expire_time']-time()) );
		...
		if ($result)
		{
			...
			//todo更新数据库
			if($data){
				$this->database->update("token", array("expire_time"=>(time()+$expire),"token"=>$this->jsapi_ticket),array("AND" => array("appid"=>$appid,'type'=>'ticket')));
			}else{
				$this->database->query("INSERT INTO iweite_huace_token (expire_time,token,appid,type) VALUES ('".(time()+$expire)."','$this->jsapi_ticket','$appid','ticket')");
			}
			return $this->jsapi_ticket;
		}
		return false;
	...
	/**
	 * 获取微信卡券api_ticket   首先从缓存里找，找不到再到数据库里找，再找不到就从微信服务器获取
	 * @param string $appid 用于多个appid时使用,可空
	...
	public function getJsCardTicket($appid='',$api_ticket=''){
		...
		//从数据库中获取
		//todo
		$data = $this->database->get("token",array("expire_time","token(CardTicket)"),array("AND" => array("appid"=>$appid,'type'=>'CardTicket') ));
		if($data){
			if($data['expire_time']>time()){
				$this->api_ticket = $data['CardTicket'];
				//设置缓存 缓存时间 = 有效时间-当前时间
				$this->setCache($authname,$this->api_ticket,($data['expire_time']-time()) );
		...
		if ($result)
		{
			...
			//todo 更新数据库
			if($data){
				$this->database->update("token", array("expire_time"=>(time()+$expire),"token"=>$this->api_ticket),array("AND" => array("appid"=>$appid,'type'=>'CardTicket')));
			}else{
				$this->database->query("INSERT INTO iweite_huace_token (expire_time,token,appid,type) VALUES ('".(time()+$expire)."','$this->api_ticket','$appid','CardTicket')");
			}
			return $this->api_ticket;
		}
		return false;
