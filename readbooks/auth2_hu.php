...
require_once IWEITE_ROOT . "./include/common_new.inc.php";
...
if (!empty($state) && ($state == 'wxauthstate'))
{
    ...
    {   
        ...
        {
            $sql = "insert into {$tablepre}openid(appid, snsapi, openid, unionid, access_token, expires_in, refresh_token) value('".$appid. "',1,'".$openid."','".$unionid."','" .$tokeninfo['access_token']."'," .time(). ", '" .$tokeninfo['refresh_token']."')";
            //exit($sql);
...
else 
{
    ...
    if (!empty($openid))
    {
        $rs = $dbauthtmp->fetch_first("select tid, snsapi, openid, username, sex, face, city, country, province from {$tablepre}openid where appid='$appid' and openid='$openid' order by tid desc limit 1");
        if ($rs)
        {
    	   //非静默授权	
    	   if ($rs['snsapi'] == 1)
