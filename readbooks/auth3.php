...
else 
{
    ...
    if (!empty($openid))
    {
        $rs = $db->fetch_first("select tid, openid, username, sex, face, city, country, province from {$tablepre}openid where appid='$appid' and openid='$openid' order by tid desc limit 1");
        if ($rs)
        {
            $userinfo["tid"] = $rs['tid'];
