...
$user = $database->query("select * from `iweite_huace_openid` where openid='$from_openid' order by tid desc limit 1")->fetchAll();
if(count($user)>0)
    $user = $user[0];
if(!$user){
    $database->query("insert into `iweite_huace_openid`(openid,dateline,username,face) value ('$from_openid','$timestamp','".$user_info['nickname']."','".$user_info['headimgurl']."')");
}
else
{
    if ((($user['username'] == '匿名') && ($user_info['nickname'] != '匿名')) || ($user['username'] == '') && ($user_info['nickname'] != '') )
    {
        //$headimgurl=$_WEITE['web_weburl']."assets/images/logo.jpg";
        $db->query("update `iweite_huace_openid` set username= '".$user_info['nickname']."',face = '".$user_info['headimgurl']."' where openid= '$from_openid'");
    }
}
...
switch ($type) {
    case Wechat::MSGTYPE_TEXT://文本消息
        ...
        if(count($result)>=1){
            ...
            $process222->process($table_suffix,$reply_id,$weObj,$database,$domains,$_WEITE['appid']);
        }else{
            //$bot_flag = 1;//机器人开关
            if($bot_flag){
...
function callBot($keyword,$bot_app_key,$bot_app_secret){
    $bot = new XiaoiBot( [ 'app_key' => $bot_app_key, 'app_secret' => $bot_app_secret ] );
